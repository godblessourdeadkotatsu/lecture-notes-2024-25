...
\begin{equation*}
	\ubracketthin{Y}_{\text{column vector}}=\ubracketthin{\mathbf{X}}_{\text{ matrix}}\beta+\ubracketthin{\varepsilon}_{\text{row vector}}.
\end{equation*}
\begin{itemize}
...
\begin{equation*}
	Y=\mathbf{X}\beta+\varepsilon\tag*{\faModx}\label{modx}
\end{equation*}
we know that:
\begin{itemize}
	\item $\ev{\varepsilon}=\ev{X\beta+\varepsilon}=X\beta$;
	\item $\var\left(Y\right)=\var\left(X\beta+\varepsilon\right)=\sigma^{2}\mathbf{I}_{n}$.
\end{itemize}
...
\end{equation*}
is the \emph{projection matrix on the space $C(x)$}. \begin{remark}
	We can say something more about the projection matrix $\mathbf{P}$:
	\begin{itemize}
		\item $\mathbf{P}$ is symmetric and idempotent;
		\item we have that
		\begin{equation*}
			\mathsf{rank}(\mathbf{P})=\mathsf{trace}(\mathsf{P})=\mathsf{trace}\left(\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}\mathbf{X}\right)=\mathbf{P};
		\end{equation*}
		\item we can decompose $Y$ into two terms: $\widehat{\mu}$ (fitted) and \emph{residual}:
		\begin{equation*}
			Y-\widehat{\mu}=Y-\mathbf{X}\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}Y=\left(\mathbf{I}_{n}-\mathbf{P}\right)Y
		\end{equation*}
		where $\left(\mathbf{I}_{n}-\mathbf{X}\right)$ is a projection matrix;
		\item $\left(Y-\widehat{\mu}\right)$ is orthogonal to all the elements of $C(x)$. Take $a$:
		\begin{align*}
			\left(\mathbf{X}a\right)^{\trsp}\left(Y-\widehat{\mu}\right)&=\left(\mathbf{X}a\right)^{\trsp}Y-\left(\mathbf{X}a\right)^{\trsp}\widehat{\mu}\\
			&=a^{\trsp}\mathbf{X}^{\trsp}Y-\left[a^{\trsp}\mathbf{X}^{\trsp}\mathbf{X}\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}Y\right]\\
			&=a^{\trsp}\mathbf{X}^{\trsp}\left[Y-\mathbf{X}\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}Y\right]=c
		\end{align*}
		and the rank of $\left(\mathbf{I}_{n}-\mathbf{P}\right)$ is $(n-p)$.
	\end{itemize}
\end{remark}
So we have
\begin{align*}
	\ev{\widehat{\beta}}&=\ev{\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}Y}\\
	&=\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}\ev{Y}\\
	&=\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}\mathbf{X}\beta\\
	&=\beta
\end{align*}
so the least-square estimators for $\beta$ are unbiased. We also find that
\begin{equation*}
	\ev{\widehat{\mu}}=\mu
\end{equation*}
so the fitted values are unbiased as well. We have
\begin{align*}
	\var\left(\widehat{\beta}\right)&=\var\left(\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}Y\right)\\
	&=\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}\var\left(Y\right)\left(\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}\right)^{\trsp}\\
	&=\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}\sigma^{2}\mathbf{I}_{n}\mathbf{X}\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\\
	&=\sigma^{2}\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}.
\end{align*}
Moreover
\begin{align*}
	\var\left(\widehat{\beta}\right)&=??
\end{align*}
So knowing that $	\var\left(\widehat{\beta}\right)=\sigma^{2}\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}$ and that $\ev{\widehat{\beta}}=\beta$ is it true to say that $\var\left(\widehat{\beta}\right)\xrightarrow{n\to\infty}0$? Consistency in mean square holds if and only if the diagonal elements of $\mathbf{V}=\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}$ go to 0 as $n\to\infty$.
\begin{remark}
	So fare we have considered controlled experiments where the design matrix $\mathbf{X}$ is fixed. In many experiments, though, we may have $Y$ and $\mathbf{X}$ of the same type (the design is random \textit{and} the response is random). For instance consider $Y$ the measurement of weight and $\mathbf{X}$ the measurement of weight. In this case we have to condition on $X$ and it is called \emph{random design}. Moreover, it is an unrealistic setting to have
	\begin{equation*}
		\left(Y_{1},X_{1}\right),\ldots,(Y_{n},X_{n})
	\end{equation*}
	and more realistically we have
	\begin{equation*}
		\left(Y_{1},X_{\pi(1)}\right),\ldots,\left(Y_{n},X_{\pi(n)}\right)
	\end{equation*}
	where $\pi(\cdot)$ is a permutation. This is called \emph{unmatched linear regression}. Another variation could introducing a noise in the $\mathbf{X}$ variable instead of just affecting the measurements $Y$: in this case our structure is typically like this:
	\begin{equation*}
		\begin{larray}
			x=x^{\star}+\eta\\
			y=\beta x^{\star}+\varepsilon
		\end{larray}
	\end{equation*}
	with $x^{\star}$ being the true explanatory variable (which we typically not observe). This is related to the topic of \emph{deconvolution}.
\end{remark}
\subsection{Estimating $\sigma^{2}$}
We don't have much information: we only know that
\begin{align*}
	\ev{\varepsilon^{2}_{i}}=\var\left(\varepsilon_{i}\right)=\sigma^{2}.
\end{align*}
Under the second order assumptions we cannot write the likelihood (we don't have the density function). It is reasonable to estimate $\sigma^{2}$ by the empirical mean of $\widehat{\varepsilon}_{i}^{2}$ where $\widehat{\varepsilon}_{i}$ is the component of order $i$ of
\begin{equation*}
	\widehat{\varepsilon}=Y-\widehat{\mu}.
\end{equation*}
We define this estimator for $\sigma^{2}$ as
\begin{equation*}
	\widehat{\sigma}^{2}=\frac{1}{n}\sum_{i=1}^{n}\widehat{\varepsilon}^{2}_{i}=\frac{\norm{\widehat{\varepsilon}}^{2}}{n}.
\end{equation*}
Remember that this is a moment-based estimator! I can write
\begin{align*}
	\norm{\widehat{\varepsilon}}^{2}&=\norm{Y-\widehat{\mu}}^{2}\\
	&=Q\left(\widehat{\beta}\right)\\
	&=\left(Y-\widehat{\mu}\right)^{\trsp}\left(Y-\widehat{\mu}\right)\\
	&=Y^{\trsp}\left(\mathbf{I}_{n}-\mathbf{P}\right)^{\trsp}\left(\mathbf{I}_{n}-\mathbf{P}\right)Y\\
	&=Y^{\trsp}\left(\mathbf{I}_{n}-\mathbf{P}\right)Y
\end{align*}
so remember that
\begin{align*}
	\ev{n\widehat{\sigma}^{2}}&=\ev{Y^{\trsp}\left(\mathbf{I}_{n}-\mathbf{P}\right)Y}\\
	&=\mu^{\trsp}\left(\mathbf{I}_{n}-\mathbf{P}\right)\mu+\mathsf{trace}\left(\left(\mathbf{I}_{n}-\mathbf{P}\right)\sigma^{2}\mathbf{I}_{n}\right)\\
	&=\mathsf{trace}\left(\left(\mathbf{I}_{n}-\mathbf{P}\right)\sigma^{2}\mathbf{I}_{n}\right)\\
	&=\sigma^{2}(n-p)
\end{align*}
so the estimator $\widehat{\sigma}^{2}$ is squared! But since it is squared up to a constant we can construct an unbiased estimator by taking
\begin{equation*}
	s^{2}=\widehat{\sigma}^{2}\frac{n}{n-p}.
\end{equation*}
For example take $p=1$ and $X=\mathbf{1}_{n}$. We have
\begin{equation*}
	Y=\mathbf{1}_{n}\beta+\varepsilon
\end{equation*}
where $\beta$ is a scalar. The moment based estimator is
\begin{equation*}
	\widehat{\beta}=\frac{1}{n}\sum_{i=1}^{n}Y_{i}
\end{equation*}
and
\begin{equation*}
	\widehat{\beta}=\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}\mathbf{X}^{\trsp}Y
\end{equation*}
but don't forget that in our case
\begin{equation*}
	\begin{carray}
		\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}=\frac{1}{n}\qquad \mathbf{X}^{\trsp}Y=\sum_{i=1}^{n}Y_{i}\\
		\Downarrow\\
		\widehat{\beta}=\frac{1}{n}\sum_{i=1}^{n}Y_{i}.
	\end{carray}
\end{equation*}
So the normal MME is covered by the linear framework. Another example: let's add one dimension and take $p=2$ with
\begin{equation*}
	\mathbf{X}=\begin{bmatrix}
		...
		1&x_{n}
	\end{bmatrix}
\end{equation*} being a $n\times 2$ matrix. We have
\begin{equation*}
	\left(\mathbf{X}^{\trsp}\mathbf{X}\right)=\begin{bmatrix}
		n&\sum_{i=1}^{n} x_{i}\\
...
and 
\begin{equation*}
	\left(\mathbf{X}^{\trsp}\mathbf{X}\right)^{-1}=\frac{1}{n\sum_{i=1}^{n}x_{i}^{2}-\left(
		\sum_{i=1}^{n}x_{i}\right)^{2}}\begin{bmatrix}
		\sum_{i=1}^{n}x^{2}_{i}&-\sum_{i=1}^{n}x_{i}\\
		-\sum_{i=1}^{n}&n
	\end{bmatrix}
\end{equation*}
and 
\begin{equation*}
	\mathbf{X}^{\trsp}Y=\begin{bmatrix}
		\sum_{i=1}^{n}y_{i}\\
...
\begin{equation*}
	\widehat{\beta}=\begin{bmatrix}
		\ybar-\frac{s_{xy}}{s_{xx}}\xbar\\
		\frac{s_{xy}}{s_{xx}}
	\end{bmatrix}
\end{equation*}
with
\begin{equation*}
	\ybar=\frac{1}{n}\sum_{i=1}^{n}y_{i}\qquad\xbar=\frac{1}{n}
\end{equation*}
and
\begin{equation*}
	s_{xx}=\sum_{i=1}^{n}\left(x_{i}-\xbar\right)^{2}\qquad s_{xy}=\sum_{i=1}^{n}\left(x_{i}-\xbar\right)\left(y_{i}-\ybar\right).
\end{equation*}
To evaluate the quality of estimators we have:
\begin{itemize}
	\item \emph{sample correlation}:
	\begin{equation*}
		r=\frac{s_{xy}}{\sqrt{s_{xx}s_{xy}}}
	\end{equation*}
	which is a descriptive index that quantifies the level of closeness between data points and fitted values;
	\item \emph{coefficient of determination}:
	\begin{equation*}
		r^{2}=\frac{s^{2}_{xy}}{s_{xx}s_{xy}}
	\end{equation*}
	which measures how good we are doing by fitting the data to a linear model; it is the fraction of variability of the $y_{i}$ which can be imputed to the linear component of the model.
\end{itemize}
%\input{tutorials}
\end{document}
