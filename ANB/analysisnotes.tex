...
\end{fancyproof}
\begin{definition}
	If $(X,\dnorm_X)$ and $(Y,\dnorm_Y)$ are two normed spaces, we denote by $\mathcal{L}(X,Y)$ the space of continuous linear applications from $X$ to $Y$. If $X=Y$ se simply denote $\mathcal{L}(X)$.
\end{definition}
\end{document}
