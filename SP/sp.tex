...
\begin{figure}[h]
	...
	\begin{tikzpicture}[framed]
		\draw[thick] (0,0) circle (1.5cm) node[anchor=east] {$A$}
		(2,0) circle (1.5cm) node[anchor=west] {$B$} (1,-1.5) circle (1.5cm) node[anchor=north] {$C$};
		\node[anchor=south,yshift=-2cm] at (current bounding box.north) {$A \cup B$};
	\end{tikzpicture}
\end{figure}
