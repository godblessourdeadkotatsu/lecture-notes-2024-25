...
So let's have a fucking party!
		...
		\hfill Kotatsu
		\vskip1.2cm
		Check the source code for this and other notes in my GitHub repo $\to$ \href{https://github.com/godblessourdeadkotatsu/lecture-notes-2024-25/tree/main/SP}{\faGithubSquare}
	\end{preface}
	\clearpage
	\tableofcontents
...
\begin{enumerate}[\circnum]
	...
	\begin{fancyproof}
		\begin{enumerate}
			...
			\begin{align*}
				W_{t_{j}}-W_{t_{j-1}}&=B_{t_{j}+a}-\cancel{B_{a}}-B_{t_{j-1}+a}+\cancel{B_{a}}\\
...
\begin{enumerate}
		...
		\begin{align*}
			\ev{|B_{t}|}&=\int_{0}^{\infty}f_{|B_{t}|}(y)\dy\\
...
\dif\left(e^{-\frac{y^{2}}{2t}}\right)
	...
	\begin{align*}
		...
		&=e^{\frac{t}{2}|\lambda^{2}|}e^{i\inprod{\lambda, B_{s}}}\evs\Bigl[e^{i\inprod{\lambda,\overbracket{B_{t}-B_{s}}^{B_{t-s}}}}\Bigr]\\
...
and
\begin{equation*}
	\ev{B_{\tau_{1}}}=1\text{ and }\evs B_{\tau_{1}}=\evs B^{2}_{\tau_{1}}=1.
\end{equation*}
This means that
\begin{equation*}
	\evs{\tau}=\infty.
...
\begin{remark}
	We have 
	\begin{equation*}
		M_{t}\sim M_{t}-B_{t}\quad\every t\geq 0 \qquad\text{given and fixed.}
	\end{equation*}
\end{remark}
\begin{fancyproof}
	By symmetry,\begin{align*}
		M_{t}\sim\sup_{s\leq t}(-B_{s})&=\sup_{s\leq t}((B_{t}-B_{s})-B_{t})\\
		&=\sup_{s\leq t}\Big[\ubracketthin{(B_{t}-B_{t-s})}_{\claptext{$\widetilde{B}_{s}:$ time inversion symmetry}}\Big]-B_{t}\\
...
\begin{fancyproof}
	...
	\begin{align*}
		h(s)&=\pr(B_{u}\neq 0,\every u\in[s,t])\\
		\text{\scriptsize conditioning the $\pr$ to the value of $B$ at $s\rightarrow$}&=\int_{\R}\pr^{B_{s(\omega)}}(B_{u-s}\neq0,\every u\in[s,t])\cdot\pr(B_{s}\in\dif b)\\
	\end{align*}
	...
	\begin{align*}
		\int_{\R}\pr^{B_{s}(\omega)}(B_{u-s},\every u\in[s,t])\cdot\pr(B_{s}\in\dif b)&=\int_{\R}\ubracketthin{\pr^{b}}_{\mathrlap{\pr(\cdot\mid B_s(\omega)=b)}}(B_{u-s}\neq0,\every u\in[s,t])f_{B_{s}}(b)\dif b\\
		&=\int_{\R}\pr^{0}(B_{u-s}\neq-b,\every u\in[s,t])f_{B_{s}}(b)\dif b\\
		&=\int_{\R}\pr^{0}(B_{v}\neq-b,\every v\in[0,t-s])f_{B_{s}}(b)\dif b\\
		&=\int_{\R}\pr^{0}(\tau_{-b}>t-s)\ubracketthin{f_{B_{s}}(b)}_{\mathclap{\frac{1}{\sqrt{2\pi s}}e^{-\frac{b^{2}}{2s}}}}\dif b.\tag*{\faCocktail}\label{cock}
	\end{align*}
	Note that
	\begin{equation*}
		\pr^{0}(\tau_{b}\geq t-s)=2\frac{1}{\sqrt{2\pi(t-s)}}\int_{0}^{b}e^{-\frac{x^{2}}{2(t-s)}}\dx
	\end{equation*}
	so
	\begin{equation*}
		\eqref{cock}=2\int_{0}^{\infty}\left(\frac{1}{\sqrt{2\pi(t-s)}}\int_{0}^{b}e^{-\frac{x^{2}}{2(t-s)}}\dx\right)\cdot\frac{1}{\sqrt{2\pi s}}\cdot e^{-\frac{b^{2}}{2s}}\dif b.
	\end{equation*}
...
\begin{theorem}
	...
	\begin{equation*}
		V_{n}=\sum_{(s,t]\in\mathcal{A}_{n}}|B_{t}-B_{s}|^{2}
	\end{equation*}
...
\begin{fancyproof}
	...
	\end{equation*}
	by convergence of quadratic variation. But this impossible because we know that the quadratic variation of the \bwm{} must be $b-a$. \par
	This holds of $\every\omega$ in the intersection of $\Omega_{ab}$, $\every$ rational $a,b|a<b$. 
\end{fancyproof}
\begin{remark}
...
This is like saying
\begin{equation*}
	\limsup\frac{B_{t}}{\sqrt{t}}=+\infty\qquad\liminf\frac{B_{t}}{\sqrt{t}}=-\infty
\end{equation*}
...
\begin{remark}
	...
	\begin{enumerate}
		\item it has strong Markov property;
		\item $\lim_{h\searrow0}\frac{1}{h}\pr(|\Delta_{h}X(t)|>\varepsilon|X(t)=x)=0$, $x\in I$;
...
\begin{definition}
	...
	\end{equation*}
	For diffusion processes these infinitesimal moments are zero for r>2.
\end{definition}
\begin{lemma}
...
\end{enumerate}
A diffusion process which is often used is the \emph{Ornstein-Uhlenbeck} process:
\begin{equation*}
...
\end{equation*}
Not considering the noise (that in this model is represented by the \bwm{} component) we describe the phenomenon as a simple first order differential equation that we can quite easily solve. Then we add back the noise (that contains all stochastic elements) and so we have our perturbed model.\par
There are other methods to ``build'' new diffusions.
\begin{theorem}
	Let $X(t)$ be a regular diffusion process on $I=(l,r)$ (it makes no difference open or closed). Let $\mu(x)$ and $\sigma^{2}(x)$ be its infinitesimal moments. Let $g(x)$ be a strictly monotone function on $I$ with continuous first and second derivatives $g'(x)$ and $g''(x)$ for $x\in(l,r)$. Then the transformed process
	\begin{equation*}
		Y(t)=g(X(t))
	\end{equation*}
	...
	\begin{equation*}
		\begin{array}{c}
			...
			\sigma^{2}_{Y}(y)=\sigma^{2}(x)g'(x)\big|_{x=g^{-1}(y)}
		\end{array}
	\end{equation*}
	Where $y=g(x)$.
\end{theorem}
...
\begin{enumerate}[\circnum]
	...
	\begin{equation*}
		v(x)=\ev{T^{\star}|X(0)=x}\qquad a<x<b.
	\end{equation*}
...
So
\begin{equation*}
	\mathfrak{L}f(x)=\unmezz\frac{\dif}{\dif M(x)}\left[\frac{\dif f(x)}{\dif S(x)}\right].
\end{equation*}
...
\begin{enumerate}[\bfseries A:]
	...
	\begin{equation*}
		\unmezz\frac{\dif}{\dif M(x)}\left[\frac{\du(x)}{\dif S(x)}\right]=0\qquad\begin{array}{l}
			x\in(a,b)\\
			u(a)=0\\
			u(b)=1.
		\end{array}
	...
	Integrate twice:
	\begin{equation*}
		\frac{\dif u(x)}{\dif S(x)}=\beta\implies u(x)=\alpha+\beta S(x)\quad x\in(a,b).
	\end{equation*}
	We use the boundary conditions:
	\begin{equation*}
		\begin{array}{>{\displaystyle}c}
			u(a)=0\implies \alpha=-\beta S(a)\\
...
\end{figure}
It is possible to prove that $\psi(x)$ satisfies the {Fokker-Plank} equation with $\frac{\partial f}{\partial t}=0$ since we know that the distribution doesn't change anymore as $t$ changes:
\begin{equation*}
...
\begin{lemma}
	\begin{enumerate}
		...
		\item If $S(l,x_{0}]=\infty$ for some $x_{0}\in(l,r)$ then
		\begin{equation*}
			\pr(T_{l^{+}}\leq T_{b}|X(0)=x)=\infty\qquad\every l<x<b<r.
		\end{equation*}
	\end{enumerate}
\end{lemma}
\begin{definition}
...
Remember that 
\begin{equation*}
	\Sigma(l)=\int_{l}^{x}S(l,\xi]\dif M(\xi)=\int_{l}^{a}M[\eta,x]\dif S(\eta)
\end{equation*}
...
\end{enumerate}
\subsection{Levy's original argument}
Let us assume that we have simulated the \bwm{} on a time grid of size $n$. If we want to generate a \bwm{} on a thinner time grid we need to do everything again from scratch which is shit. To avoid this, we work with time grids of $2^{n}$ and us Brownian bridges.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{img/screenshot071}
	\caption{To ``add'' new information between two points we use a Brownian bridge.}
	\label{fig:screenshot071}
\end{figure}
We want to describe the value of $B_{\frac{2k+1}{2^{n+1}}}$ when $B_{\frac{k}{2^{n}}}$ and $B_{\frac{k+1}{2^{n}}}$ are known.
\begin{proposition}
	If $X\distnorm{0,\sigma_{1}^{2}}\independent Y\distnorm{0,\sigma_{2}^{2}}$ then
	\begin{equation*}
		X|X+Y\distnorm{\frac{(X+Y)\sigma_{1}^{2}}{\sigma_{1}^{2}+\sigma_{2}^{2}},\frac{\sigma_{1}^{2}\sigma_{2}^{2}}{\sigma_{1}^{2}+\sigma_{2}^{2}}}.
	\end{equation*}
\end{proposition}
\begin{fancyproof}
	We have $X+Y\distnorm{0,\sigma_{1}^{2}+\sigma_{2}^{2}}$ so
	\begin{equation*}
		\ev{f(X)g(X+Y)}=\frac{1}{\sqrt{2\pi}\sigma_{1}\sigma_{2}}\int\int f(x)g(x+y)e^{-\frac{x^{2}}{2\sigma_{1}^{2}}-\frac{y^{2}}{2\sigma_{2}^{2}}}\dx\dy.
	\end{equation*}
	...
	and get 
	\begin{equation*}
		\ev{f(X)g(X+Y)}=\frac{1}{\sqrt{2\pi}\sigma_{1}\sigma_{2}}\int\int f(y)g(v)e^{-\frac{u^2}{2\sigma^{2}_{1}}-\frac{(v-y)^{2}}{2\sigma_{2}^{2}}}\dif v\du
	\end{equation*}
	so the conditional distribution is
	\begin{equation*}
		\begin{array}{>{\displaystyle}c}
			\frac{\sqrt{\sigma_{1}^{2}+\sigma_{2}^{2}}}{\sqrt{2\pi}\sigma_{1}\sigma_{2}}\expg{-\frac{u^{2}}{2\sigma_{1}^{2}}+\frac{v^{2}}{2\left(\sigma_{1}^{2}+\sigma_{2}^{2}\right)}-\frac{v^{2}}{2\sigma_{2}^{2}}-\frac{u^{2}}{2\sigma_{1}^{2}}+\frac{2uv}{2\sigma_{2}}}\\
			\Downarrow\\
			\frac{\sqrt{\sigma_{1}^{2}+\sigma_{2}^{2}}}{\sqrt{2\pi}\sigma_{1}\sigma_{2}}\expg{-\unmezz\left(u-\frac{v\sigma_1^{2}}{\sigma_{1}^{2}+\sigma_{2}^{2}}\right)^{2}\frac{\sigma_{1}^{2}+\sigma_{2}^{2}}{\sigma_{1}^{2}\sigma_{2}^{2}}}.
		\end{array}
	\end{equation*}
\end{fancyproof}
In the Brownian case we have that the sum $B_{t_{3}}-B_{t_{1}}$ is known and
\begin{equation*}
	\ubracketthin{B_{t_{3}}-B_{t_{2}}}_{\mathsf{N}(0,t_{3}-t_{2})}\independent\ubracketthin{B_{t_{2}}-B_{t_{1}}}_{\mathsf{N}(0,t_{2}-t_{1})}.
\end{equation*}
\begin{proposition}
	We have that
	\begin{equation*}
		B_{t_{2}}-B_{t_{1}}\mid B_{t_{3}}-B_{t_{1}}\distnorm{\frac{(B_{t_{3}}-B_{t_{1}})(t_{2}-t_{1})}{t_{3}-t_{1}},\frac{(t_{2}-t_{1})(t_{3}-t_{1})}{t_{3}-t_{1}}}.
	\end{equation*}
	So we have
	\begin{equation*}
		B_{t_{2}}\sim B_{t_{1}}+\frac{t_{2}-t_{1}}{t_{3}-t_{1}}\left(B_{t_{3}}-B_{t_{1}}\right)+\sqrt{\frac{(t_{2}-t_{1})(t_{3}-t_{2})}{t_{3}-t_{1}}}\ubracketthin{G}_{\mathclap{\distnorm{0,1}}}.
	\end{equation*}
\end{proposition}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{img/screenshot072}
	\caption{Another very necessary figure}
	\label{fig:screenshot072}
\end{figure}
\subsection{Karhunen-Loeve theorem}
We know that if 
	${(X_{t})}_{t\geq0}\text{ is a \bwm}\implies(X_{t})\text{ is a Gaussian}$ continuous process with covariance function $K(s,t)=s\wedge t$. Let us construct the linear operator $T_{k}:L^{2}([a,b])\to L^{2}([a,b])$:
	\begin{equation*}
		T_{k}(f)(t)=\int_{a}^{b}K(s,t)f(s)\ds.
	\end{equation*} 
	We can see that $T_{k}$ is a probability kernel. Since it is a linear operator we can use the spectral decomposition and compute eigenfunctions and eigenvalues $(e_{k},\lambda_{k})$.
\begin{theorem}
	(Mercer). Since $K$ is symmetric positive semidefinite then ${(e_{k})}_{k}$ is an orthogonal basis and $K$ has the representation
	\begin{equation*}
		K(s,t)=\sum_{k=1}^{\infty}\lambda_{k}e_{k}(s)e_{k}(t).
	\end{equation*}
\end{theorem}
We construct 
\begin{equation*}
	X_{t}=\sum_{k=1}^{\infty}\sqrt{\lambda_{k}}G_{k}e_{k}(t)
\end{equation*}
where $G_{k}$ is a sequence of i.i.d. Gaussian random variable $\mathsf{N}(0,1)$. $X_{t}$ converges in $L^{2}$. Moreover
\begin{align*}
	\ev{X_{t}X_{s}}&=\sum_{k,j=1}^{\infty}\sqrt{\lambda_{k}\lambda_{j}}\ubracketthin{\ev{G_{k}G_{j}}}_{\claptext{0 if $k\neq j$}}e_{k}(t)e_{j}(s)\\
	&=\sum_{k=1}^{\infty}\lambda_{k}e_{k}(t)e_{k}(s)\\
	&=K(s,t).
\end{align*}
So, to recap, we have 2 approaches to generate a \bwm:
\begin{enumerate}
	\item using a time grid $0\leq t_{0}\leq\ldots\leq t_{n}$. This suffices to generated the Gaussian vector $(B_{t_{1}},\ldots,B_{t_{n}})$ with covariance matrix $K(i,j)=\frac{T}{n}\min\{i,j\}$ where $i=\frac{t_{i}}{n}$ and $j=\frac{t_{j}}{n}$. If a matrix $A$ satisfies $K=AA^{\trsp}$ (Cholesky factorization of $A$) then 
	\begin{equation*}
		(B_{t_{1}},\ldots,B_{t_{n}})\sim A\cdot G
	\end{equation*}
	with $G\distnorm{0,1}$. We have two extensions.
	\begin{enumerate}
		\item The Ornstein-Uhlenbeck process ${(X_{t})}_{t\geq0}$ is a continuous Gaussian process with 
		\begin{equation*}
			\begin{array}{l}
				X_{0}\distnorm{0,1}\\
				\ev{X_{t}}=0\\
				\Gamma(s,t)=\exp\{-\ubracketthin{\gamma}_{>0}|t-s|\}.
			\end{array}
		\end{equation*}
		\item the fractional \bwm ${(B_{t}^{\mathsf{H}})}_{t\geq0}$ is a continuous Gaussian process satisfying 
		\begin{itemize}
			\item $B^{\mathsf{H}}_{0}=0$;
			\item $\ev{B^{\mathsf{H}}}$;
			\item $\ev{B_{t}^{\mathsf{H}}B_{s}^{\mathsf{H}}}=\unmezz\left(|t|^{2\mathsf{H}}+|s|^{2\mathsf{H}}-|t-s|^{2\mathsf{H}}\right)$.
		\end{itemize} 
		Prove that for $\mathsf{H}=\unmezz$, $B^{\mathsf{H}}$ is a \bwm.
	\end{enumerate}
	\item Another approach is having $K$ decomposed in an orthonormal basis of $L^{2}([a,b])$:
	\begin{align*}
		K(s,t)=\sum_{k=1}^{\infty}\lambda_ke_k(s)e_k(t)
	\end{align*}
	where
	\begin{equation*}
		X_t=\sum_{k=1}^{\infty}\sqrt{\lambda_k}G_ke_k(t)
	\end{equation*}
	defines the continuous Gaussian process with covariance function $K(s,t)$.
\end{enumerate}
Let's apply this to the \bwm{} on the interval $(0,1]$: first find the eigenvalues:
\begin{align*}
	\int_{0}^{1}e(s)\ds&=\lambda e(t)\qquad t\in[0,1]\\
	\implies\int_{0}^{1}(s\wedge t)e(s)\ds&=\lambda e(t)\\
	\implies\int_{0}^{t}se(s)\ds+t\int_{t}^{1}e(s)\ds&=\lambda e(t).
\end{align*}
We take the derivative to get
\begin{equation*}
	te(t)+\int_{t}^{1}e(s)\ds-te(t)=\lambda e'(t).
\end{equation*}
We observe that $e'(1)=0$ and $e(0)=0$. Take the derivative again
\begin{align*}
	-e(t)&=\lambda e''(t)\\
	\implies e(t)&=A\sin\left(\frac{t}{\sqrt{\lambda}}\right)+B\cos\left(\frac{t}{\sqrt{\lambda}}\right)
\end{align*}
and we get 
\begin{equation*}
	\begin{cases}
		e_k(t)=\sqrt{2}\sin\left(\left(k-\unmezz\right)\pi t\right)\\
		\lambda_k=\frac{1}{\left(k-\unmezz\right)^{2}}\pi^{2}.
	\end{cases}
\end{equation*}
The \bwm{} satisfies
\begin{equation*}
	B_t=\sqrt{2}\sum_{n=1}^{N}G_k\frac{\sin\left(k-\unmezz\pi t\right)}{\left(k-\unmezz\right)\pi}.
\end{equation*}
\subsection{Simulation of $d$-dimensional \bwm}
We have 
\begin{equation*}
	B_{t}=\left(B_{t}^{1},B_{t}^{2},\ldots,B_{t}^{d}\right)
\end{equation*}
that corresponds to $d$ independent \bwm s.
\begin{equation*}
	B_{t}-B_{s}\distnorm{\mathbf{0},(t-s)I^{d}}
\end{equation*}
with $I^{d}$ being the identity matrix of dimension $d\times d$. 
\begin{definition}
	A \emph{$Q$-\bwm{}} is a $d$-dimensional \bwm{} such that
	\begin{equation*}
		B_{t}-B_{s}\distnorm{\mathbf{0},(t-s)Q}
	\end{equation*}
	where $Q$ is a $d\times d$ matrix.
\end{definition}
To generate a $Q$-\bwm:
\begin{enumerate}
	\item find $A$ such that $A^{\trsp}A=Q$;
	\item generate a $d$-dimensional \bwm;
	\item $A((B_{t}^{1},B_{t}^{2},\ldots,B_{t}^{d})$ is a $Q$-\bwm.
\end{enumerate}
\begin{exercise}
	Find a $Q$-\bwm{} with
	\begin{equation*}
	...
	\end{equation*}
\end{exercise}
The $Q$-\bwm{} is a ``well behaving'' \bwm which doesn't go craaazy in all directions. But I will, very soon. \par
Sometimes... Our \bwm{} is not really a \bwm. 
\begin{itemize}
	\item The electric potential of neurons looks like a \bwm{} but its is actually bounded between 0 and 1. Also, neurons have memory.
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.4\linewidth]{img/screenshot073}
		\caption{Blud was lacking}
		\label{fig:screenshot073}
	\end{figure}
	\item The price of a financial asset looks like a \bwm{} but actually we can see that its volatility depends on the price. Lower prices generate smaller volatility, higher prices generate more volatility.
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.4\linewidth]{img/screenshot074}
		\caption{Blud was lacking AGAIN}
		\label{fig:screenshot074}
	\end{figure}
	It is more connected with the exponential of the \bwm.
\end{itemize}
\section{The SDE (or: how I learned not to worry and to refuse solving Cauchy problems)}
\subsection{A brief overview}
For differential equations, the initial value problem is
\begin{equation*}
	\begin{cases}
		x=\frac{\dx}{\dt}=a(t,x(t))\\
		x(t_{0})=x_{0}
	\end{cases}\tag*{\faWhatsapp}\label{initialvalue}.
\end{equation*}
In general, there are no explicit expressions.
\begin{theorem}
	\emph{Cauchy-Lpischitz}. Let $U\subset\R^{d}$ be an open set and $a:[0,T]\times U\to\R^{d}$ be a Lipschitz continuous function. Then $\existsAlt M$ such that
	\begin{equation*}
		|a(t,x)-a(t,y)|\leq M|x-y|\qquad\every(t,x),(t,y)\in[0,T]\times U.
	\end{equation*}
\end{theorem}
So for any $x_0\in U$ there exists a unique solution to \ref{initialvalue}. How could we approximate this solution $x(t)$? \par
The basic differential method is the \emph{Euler method}. We introduce a time grid $0=t_0<t_1<\ldots<t_{n}=T$ that is regular, with a step size of $\Delta=t_k-t_{k-1}$. If $(y_0,y_1,\ldots,y_n)$ is an approximation of $x(t)$ oh the time grid we define the global error as
\begin{equation*}
	e_n=y_n-x(t_{n}).
\end{equation*}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{img/screenshot075}
	\caption{Sorry for my handwriting.}
	\label{fig:screenshot075}
\end{figure}
\begin{theorem}
	Let us assume that $\norm{\ddot{x}}$, the norm of second derivative of $x$ with respect to $t$, is bounded on $[0,T]$. Then defining
	\begin{equation*}
		y_{n+1}=y_{n}+a(t_{n},x(t_{n}))
	\end{equation*}
	we get
	\begin{equation*}
		\norm{e_n}\leq\frac{1}{2n}\left(e^{MT}-1\right)\sup\norm{\ddot{x}}\cdot \Delta.
	\end{equation*}
\end{theorem}
The Euler method is of order $1$. Methods of order $p$ have
\begin{equation*}
	\norm{e_n}=\mathcal{O}(\Delta^{p})\qquad\text{as }\Delta\to0
\end{equation*}
which means it is bounded by a constant of order $\Delta^{p}$ as $\Delta\to0$.
\begin{fancyproof}
	We have
	\begin{align*}
		x(t_{n+1})&=x(t_{n})+\dot{x}(t_{n})\Delta+\ddot{x}(\vartheta_n)\frac{\Delta^{2}}{2}\quad\vartheta_n\in[t_{n},t_{n+1}]\\
		&=x(t_{n})+a(t_{n},x(t_{n}))\Delta+\ddot{x}(\vartheta_n)\frac{\Delta^{2}}{2}.
	\end{align*}
	We also have
	\begin{align*}
		e_{n+1}&=y_{n+1}-x(t_{n+1})\\
		&=y_{n}-a(t_{n},y_{n})\Delta-x(t_{n})-a(t_{n},x(t_{n}))\Delta-\ddot{x}(\vartheta_n)\frac{\Delta^{2}}{2}.
	\end{align*}
	By the previous theorem we have
	\begin{equation*}
		\norm{e_{n+1}}\leq\norm{e_n}+\ubracketthin{M\norm{e_n}\Delta}_{\claptext{Due to Lipschitz property of $a$}}+\unmezz\sup\norm{\ddot{x}}\Delta^{2}.
	\end{equation*}
	We notice that
	\begin{equation*}
		e_0=y_0=x_0=0
	...
	so
	\begin{equation*}
		\norm{e_{n+1}}\leq\frac{(1+M\Delta)^{n+1}-1}{(1+M\Delta)-1}\unmezz\sup\ddot{x}\Delta^{2}.
	\end{equation*}
\end{fancyproof}
\begin{remark}
	It is possible to modify the Euler method:
	\begin{equation*}
		\begin{array}{rl}
			y_{n+1}=y_{n}+&\xcancel{a(t_{n},y_{n})\Delta}\\
			&a(t_{n+1},y_{n+1})\Delta.
		\end{array}
	\end{equation*}
	This is called the ``backward'' or ``implicit'' Euler method.
\end{remark}
To compute the value of $y_{n+1}$ we need to solve an equation. This method is boring as shit and horrible and I hope I will never be required to do it but it is also more stable.
\subsection{The stochastic integral}
We want to generalize the ordinary differential equations to the statistic framework. We have already seen the \emph{stochastic differential equation}
\begin{equation*}
	\dif X_{t}=b(t,X_{t})\dt+\sigma(t,X_{t})\dif B_{t}.
\end{equation*}
\begin{insult}
	I'll tell you something. This thing doesn't mean shit, it is not even an abuse of notation. This is LITERALLY a made up thing that DOESN'T MEAN ANYTHING and servers ZERO purpose if not to shorten the integral form of this equation.
\end{insult}
The integral form of this SDE, which is the ``real'' thing, is 
\begin{equation*}
	X_t=X_{0}+\ubracketthin{\int_{0}^{t}b(s,X_s)\ds}_{\text{classic Riemann-Stieltjes integral}}+\ubracketthin{\int_{0}^{t}\sigma(s,X_{s})\dif B_s}_{\text{statement of the utterly deranged}}.
\end{equation*}
It makes no sense to write $\dif B_{s}$ because... \bwm{} is nowhere differentiable. We need a new kind of integral for this. Consider the normal Riemann integral method.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\linewidth]{img/screenshot076}
	\caption{I just learned what a Lebesgue integral is for probability theory and now this shit. Crazy.}
	\label{fig:screenshot076}
\end{figure}
Here we have
\begin{equation*}
	\int_{0}^{t}f(s)\ds=\sum_{k}f(t_{k})(t_{k+1}-t_k).
\end{equation*}
The idea is using the forward Euler discretization
\begin{equation*}
	\sum_{k=0}^{N-1}B_{t_{k}}\ubracketthin{\left(B_{t_{k+1}}-B_{t_{k}}\right)}_{:=\Delta B_{n}}.
\end{equation*}
Ir we look at what happens between $t_{k}$ and $t_{k+1}$
\begin{equation*}
	\int_{t_{k}}^{t_{k+1}}B_{s}\ds\xrightarrow{\text{\tiny since BM is continuous $B_{s}\sim B_{t_{k}}$}} B_{t_{k}}\int_{t_{k}}^{t_{k-1}}\ds.
\end{equation*}
We want to compute the expectation
\begin{equation*}
	\sum_{k=0}^{N-1}\ev{B_{t_{k}}}\ev{\Delta B_{n}}=0.\tag*{\faDog}\label{dogo}
\end{equation*}
The \faBackward\; Euler method is
\begin{align*}
	\text{\ref{dogo}}&=\sum_{k=0}^{N-1}B_{t_{k+1}}\left(B_{t_{k+1}}-B_{t_{k}}\right)\\
	&=\sum_{k=0}^{N-1}\left[\left(B_{t_{k}}+B_{t_{k+1}-B_{t_{k}}}\right)\left(B_{t_{k+1}}-B_{t_{k}}\right)\right]\\
	&=\sum_{k=0}^{N-1}\ev{B_{t_{k}}\Delta B_{n}}+\ev{\Delta B^{2}_{n}}=T\neq0.
\end{align*} 
We could also use the ``intermediate rule'' (or ``trapezoidal rule'').
\begin{equation*}
	\ev{\sum\left(\frac{B_{t_{k}}+B_{t_{k+1}}}{2}\right)\left(B_{t_{k+1}}-B_{t_{k}}\right)}=\frac{T}{2}.
\end{equation*}
So, to recap we have the forward Euler discretization
\begin{equation*}
	\int_{0}^{t}f(s)\ds=\lim_{N\to\infty}\sum_{n=0}^{N-1}f(t_{n})(t_{n+1}-t_{n}).
\end{equation*}
The Euler trapezoidal method yields the same result. For a positive measure $\mu$ we have
\begin{equation*}
	\int_{0}^{t}f(s)\ds=\lim_{N\to\infty}\sum_{n=0}^{N-1}f(t_{n})\mu\left([t_{n}-t_{n+1})\right).
\end{equation*}
For the stochastic integral we need to study the limit
\begin{equation*}
	\sum_{n=0}^{N-1}f(t_{n},B_{n})(B_{t_{n+1}}-B_{t_{n}})\to\int_{0}^{1}f(s,B_{s})\dif B_{s}.
\end{equation*}
We use the forward method which defines Ito. If we use the trapezoidal method then we end up with Shatonovich's integral. Go figure.
\begin{theorem}
	Assume there exists a constraint $c>0$ such that
	\begin{equation*}
		|f(t,x)-f(s,y)|\leq c\left(|t-s|+|x-y|\right).
	\end{equation*}
	We consider two time grids of the interval $[0,T]$:
	\begin{equation*}
		\begin{array}{>{\displaystyle}l}
			\left\{t_{n}\right\}^{N}_{n=0}, t_{0}=0,t_{N}=T\\
			\left\{\bar{t}_{n}\right\}^{\bar{N}}_{n=0},\bar{t}=0,\bar{t}_{\bar{N}}=T.
		\end{array}
	\end{equation*}
	With the forward Euler approximation we get
	\begin{align*}
		I&=\sum_{n=0}^{N-1}f(t_{n},B_{t_{n}})(B_{t_{n+1}}-B_{t_{n}})\\
		\bar{I}&=\sum_{n=0}^{\bar{N}-1}f(\bar{t}_{n},B_{\bar{t}_{n}})(B_{t_{n+1}}-B_{t_{n}}).
	\end{align*}
	Then
	\begin{equation*}
		\ev{(I-\bar{I})^{2}}=\vartheta(\Delta t_{
		\max})
	\end{equation*}
	where $\Delta t_{\max}=\max\left\{t_{n+1}-t_{n},t_{n+2}-t_{n+1}\ldots\right\}$ 
\end{theorem}
\begin{fancyproof}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.4\linewidth]{img/screenshot077}
		\caption{Two different parts of the interval}
		\label{fig:screenshot077}
	\end{figure}
	We have that an interval is made of 
	\begin{equation*}
		f(t_{n},B_{t_{n}})(B_{t_{n+1}}-B_{s})+f(t_{n},B_{t_{n}})(B_{s}-B_{t_{n}}).
	\end{equation*}
	We define a finer time grid $\left\{\hat{t}_{k}\right\}=\left\{\bar{t}_{n}\right\}\cup\left\{t_{n}\right\}$ made of the union of all nodes. In this grid
	\begin{equation*}
		I-\bar{I}=\sum_{k}\Delta f_{k}\Delta B_{k}\to B_{\hat{t}_{k+1}}-B_{\hat{t}_k}
	\end{equation*}
	and
	\begin{equation*}
		\Delta f_{k}=f(t_{n},B_{t_{n}})-f(\bar{t}_{n},B_{t_{n}})\quad\text{if }\begin{cases}
			\hat{t}_{k}\in[t_{n},t_{n+1}]\\
			\hat{t}_{k}\in[t_{m},t_{m+1}]
		\end{cases}
	\end{equation*}
	hence
	\begin{align*}
		\ev{\left(I-\bar{I}\right)^{2}}&=\sum_{k,l}\ev{\Delta f_{k}\Delta f_l\Delta B_k\Delta B_l}\\
		&=2\sum_{k>l}\ev{\Delta f_k\Delta f_l\Delta B_k\Delta B_l}+\sum_k\ev{(\Delta f_k)^{2}\Delta B^{2}_k}
	\end{align*}
	due to the independence of $B_{t_{k+1}}-B_{t_{k}}$ with respect to $\F_{t_{k}}$. So we get
	\begin{equation*}
		\ev{\Delta f_k\Delta f_l\Delta B_k\Delta B_k}=\ev{\Delta f_k\Delta f_l\Delta B_l}\cdot\ubracketthin{\ev{\Delta B_k}}_{=0}
	\end{equation*}
	and therefore
	\begin{equation*}
		\ev{(\Delta f_k)^{2}\Delta B_k^{2}}=\ev{(\Delta f_k)^{2}}\ubracketthin{
		\ev{\Delta B^{2}_k}}_{t_{k-1}-t_{k}}.
	\end{equation*}
	Use Lipschitz property
	\begin{align*}
		(\Delta f_k)^{2}&\overset{\claptext{Lipschitz}}{\leq}c^{2}\left(\Delta t_{\max}+|B_{t_{n}}-B_{\bar{t}_{n}}|\right)^{2}\\
		&\leq 2c^{2}\left(\Delta t_{\max}^{2}+|B_{t_{n}}-B_{\bar{t}_{m}}|^{2}\right)
	\end{align*}
	So 
	\begin{equation*}
		\ev{(I-\bar{I})^{2}}\leq 2c^{2}\Delta t^{2}_{\max}T+2c^{2}\Delta t_{\max}T.
	\end{equation*}
	So $I$ is depending on $\Delta t_{\max}$ and it is a Cauchy sequence in a Hilbert space (a \rv{} with finite second moment) with
\begin{equation*}
		\begin{array}{l}
		\norm{X}=\sqrt{\ev{X^{2}}}\\
		\langle X,Y\rangle+\ev{XY}.
	\end{array}
\end{equation*}
$I(\Delta t_{\max})$ converges to some limit that defines the Ito integral in $L^{2}$.
\end{fancyproof}
\begin{remark}
	\begin{equation*}
		\sqrt{\ev{(I-\bar{I})^{2}}}\leq c\sqrt{\Delta t_{\max}}.
	\end{equation*}
	If $f(t,x)=f(t)$ (it does not depend on $x$) then we have faster convergence:
	\begin{equation*}
		\sqrt{\ev{(I-\bar{I})^{2}}}\leq c\Delta t_{\max}.
	\end{equation*}
\end{remark}
\begin{definition}
	A function $f:[0,1]\times\Omega\to\R$ is adapted if $f(t,0)$ only depends on events that are generated by $(B_{s},s\leq t)$.
\end{definition}
This allows us to extend the Ito integral to adapted functions. If
\begin{equation*}
	\ev{|f(t+s,\cdot)-f(t,\cdot)}\leq cs
\end{equation*}
then the conclusion of the previous theorem still holds.
\subsection{Properties of the Ito integral}
\begin{theorem}
	If $f$ and $g$ are integrable (for example being Lipschitz) then
	\begin{enumerate}[\circnum]
		\item $\int_{0}^{T}c_1f(s,\cdot)+c_2g(s,\cdot)\dif B_s=c_1\int_{0}^{T}f(s,\cdot)\dif B_s+c_s\int_{0}^{T}g(s,\cdot)\dif B_s$;
		\item $\ev{\int_{0}^{T}f(s,\cdot)\dif B_s}=0$;
		\item $\ev{\left(\int_{0}^{T}f(s,\cdot)\dif B_s\right)\left(\int_{0}^{T}g(s,\cdot)\dif B_s\right)}=\int_{0}^{T}\ev{f(s,\cdot)g(s,\cdot)}\ds$.
	\end{enumerate}
\end{theorem}
For $d$-dimensional integrals this means having
\begin{equation*}
	B_t=\left(B_t^1,B_t^2,\ldots,B_t^d\right)\quad f(t,\cdot):[0,T]\times\Omega\to\R^{d}
\end{equation*}
so that we get
\begin{equation*}
	\begin{array}{>{\displaystyle}c}
		\int_0^Tf(s,\cdot)\dif B_s=\sum_{i=1}^{d}\int_{0}^{T}f_i(s,\cdot)\dif B_s\\
		\Downarrow\\
		\int_0^Tf(s,\cdot)\dif B_s\approx\sum_{i=1}^{d}\sum_{n=0}^{N-1}f_i(t_n,\cdot)\left(B_{t_{n-1}}^{i}-B_{t_{n}}^{i}\right).
	\end{array}
\end{equation*}
We can thus apply the Euler and Milstein schemes with $d=1$.
\begin{equation*}
	X_t=X_0+\int_{0}^{t}b(s,X_s)\ds+\int_{0}^{t}\sigma(s,X_s)\dif B_s.\tag*{\faDev}\label{dev}
\end{equation*}
We assume that $b$ and $\sigma$ are Lipschitz continuous:
\begin{align*}
	|b(t,x)-b(t,y)|&\leq c|x-y|\quad\text{for }t\in[0,T]\\
	|\sigma(t,x)-\sigma(t,y)|&\leq c|x-y|\quad\text{with linear growth}\\
	|b(t,x)|^{2}&\leq c(1+|x|^{2}).
\end{align*}
Let's assume that $\ev{X^{2}_{0}}\leq\infty$. The there exists a continuous stochastic process ${(X_{t})}_{t\in[0,T]}$ adapted and satisgying 
\begin{equation*}
	\ev{\int_{0}^{T}X^{2}_{s}\ds}<\infty\text{ s.t. \ref{dev} }\pr-\as{}
\end{equation*}
Moreover this process is pathwise unique.
\begin{equation*}
	\pr\left(\sup_{0\leq t\leq T}\left|X_{t}-\widetilde{X}_{t}\right|>\delta\right)\qquad\every\delta>0.
\end{equation*}
\begin{remark}
	If $b$ and $\sigma$ do not depend on $t$
	\begin{equation*}
		\begin{cases}
			b(t,x)=b(x)\\
			\sigma(t,x)=\sigma(x)
		\end{cases}
	\end{equation*}
	then the SDE is homogeneous.
\end{remark}
Introduce the Euler scheme with $\Delta=\max\left\{t_{j}-t_{j-1}\right\}$ on the time grid:
\begin{equation*}
	X^{\Delta}_{t_{j}}=X^{\Delta}_{t_{j-1}}+b\left(t_{j-1},X^{\Delta}_{j-1}\right)\Delta+\sigma\left(t_{j-1},X^{\Delta}_{j-1}\right)\left(B_{t_{j}}-B_{t_{j-1}}\right).
\end{equation*}
Then ${\left(X^{\Delta}_{t_{j}}\right)}_{j=0,\ldots,n}$ is an approximation of $X_{t}$. $X^{\Delta}_{t}$ is a strong approximation of $X$ of order $\beta$ if
\begin{equation*}
	\ev{|X_{t}^{\Delta}-X_{t}|}\leq c\cdot\Delta^{\beta}.
\end{equation*} 
Let $g\in \mathscr{C}^{n}(\R)$. Then $X^{\Delta}$ is a weak approximation of $x$ of order $\beta$ if
\begin{equation*}
	\left|\ev{g(X_{t}^{\Delta})}-\ev{g(X_{t})}\right|\leq c\Delta^{\beta}
\end{equation*}
We are now able to compute 
\begin{equation*}
	\ev{g(X_{t})}\approx\frac{1}{N}\sum_{n=1}^{N}g\left(X_{T}^{\Delta,n}\right)
\end{equation*}
which is the application of Monte Carlo methods to the Euler scheme.
\subsection{The Euler-Maruyama method}
Our SDE is
\begin{equation*}
	\begin{cases}
		\dif X_t=b(t,X_t)\dt+\sigma(t,B_t)\dif B_t\\
		X_0\qquad\text{fixed}.
	\end{cases}
\end{equation*}
On the time interval $[0,T]$ we approximate ${(X_{t})}_{t\in[0,T]}$ by ${(X^{\Delta}_{t})}_{t\in[0,T]}$ where $\Delta=\max\left\{t_{n+1}-t_{n}\right\}$ for $0\leq n\leq N$. We use the Euler method:
\begin{equation*}
	\begin{cases}
		X^{\Delta}_{t_{n+1}}=X^{\Delta}_{t_{n}}+b(t_{n},X_{t_{n}}^{\Delta})(t_{n+1}-t_{n})+\sigma(t_{n},X^{\Delta}_{t_{n}})(B_{t_{n+1}}-B_{t_{n}})\\
		X^{\Delta}_0=x_0
	\end{cases}.
\end{equation*}
Here ${(X^{\Delta}_{t})}_{t\in[0,T]}$ is the piecewise extension of $X^{\Delta}_{t_{n}}$.
\begin{equation*}
	X^{\Delta}_{t}=X^{\Delta}_{t_{n}}\qquad\every t\in[t_{n},t_{n+1}].
\end{equation*}
\begin{theorem}
	Under the following assumptions
	\begin{align*}
		&|b(t,x)-b(t,y)|+|\sigma+(t,\alpha)-\sigma(t,y)|\leq k|a-y|\\
		&|b(t,x)|+|\sigma(t,x)|\leq k(1+|x|)\\
		&|b(t,x)-b(s,x)|+|\sigma(t,x)-\sigma(s,x)|\leq(t(1+|x|))\sqrt{t-s}
	\end{align*}
	the Euler scheme has a strong convergence of order 2:
	\begin{equation*}
		\sup_{t\in T}\ev{|X^{\Delta}_{t}-X_{t}}\leq c_{t}\sqrt{\Delta}.
	\end{equation*}
\end{theorem}
\begin{fancyproof}
	We define $n(t)=\max\left\{n:t_{n}\leq t\right\}$. We have
	\begin{align*}
		X_{t}^{\Delta}&=X^{\Delta}_{t_{n(t)}}=X_{0}+\int_{0}^{t_{n(t)}}b\left(t_{n(s)},X^{\Delta}_{t_{n(s)}}\right)\ds+\int_{0}^{t_{n(t)}}\sigma\left(t_{n(s)},X^{\Delta}_{t_{n(s)}}\right)\dif B_s.
	\end{align*}
	Define
	\begin{equation*}
		Z(t):=\sup_{t\leq T}\ev{\left|X^{\Delta}_{t}-X_{t}\right|^{2}}.
	\end{equation*}
	We have
	\begin{align*}
		X^{\Delta}_{t}-X_{t}=&\color{Gold3}\boxed{\color{black}\begin{aligned}
				&\int_{0}^{t_{n(t)}}b\left(t_{n(s)},X^{\Delta}_{t_{n(s)}}\right)-b\left(t_{n(s)},X^{\Delta}_{s}\right)\ds+\\
		&+\int_{0}^{t_{n(t)}}\sigma\left(t_{n(s)},X^{\Delta}_{t_{n(s)}}\right)-\sigma\left(t_{n(s)},X^{\Delta}_{s}\right)\dif B_s+\\
		\end{aligned}}\tag*{\Large \color{Gold3}a}\\
	&\color{SeaGreen3}\boxed{\color{black}\begin{aligned}
				&+\int_{0}^{t_{n(t)}}b\left(t_{n(s)},X^{\Delta}_{s}\right)\ds+\int_{0}^{t_{n(t)}}\sigma\left(t_{n(s)},X^{\Delta}_{s}\right)-\sigma\left(s,X_s\right)\dif B_s-\\
		\end{aligned}}\tag*{\Large \color{SeaGreen3}b}\\
		&\color{Turquoise3}\boxed{\color{black}\begin{aligned}
				&-\int_{t_{n(t)}}^{t}b(s,X_s)\ds-\int_{t_{n(t)}}^{t}\sigma(s,X_s)\dif B_s
		\end{aligned}}\tag*{\Large \color{Turquoise3}c}
	\end{align*}
	and we know that 
	\begin{equation*}
		\begin{array}{>{\displaystyle}c}
			(\textcolor{Gold3}{a}+\textcolor{SeaGreen3}{b}+\textcolor{Turquoise3}{c})^{2}\leq3\textcolor{Gold3}{a}^{2}+3\textcolor{SeaGreen3}{b}+3\textcolor{Turquoise3}{c}^{2}\\
			\Downarrow\\
			z(t)=3z_1(t)+3z_2(t)+3z_3(t)
		\end{array}
	\end{equation*}
	so by Cauchy-Schwartz we get
	\begin{align*}
		z_1(t)&\leq2\sup_{t\leq T}\int_{0}^{t_{n(t)}}\ev{\left(b\left(t_{n(s)},X^{\Delta}_{t_{n(s)}}\right)-b\left(t_{n(s)},X_{s}\right)\right)^{2}}\ds+\\
		&+2\sup_{t\leq T}\ev{\left(\sigma\left(t_{n(s)},X^{\Delta}_{t_{n(s)}}\right)-\sigma\left(t_{n(s)},X_{s}\right)\right)^{2}}\dif B_s\\
		&\overset{\text{Lipschitz}}{\leq}n c_t\int_{0}^{T}\ubracketthin{\ev{\left(X^{\Delta}_{t_{n(s)}}-X_{s}\right)^{2}}}_{\mathclap{\leq\ubracketthin{\sup_{0\leq t\leq s}\ev{\left|X^{\Delta}_{t}-X_{t}^{2}\right|}}_{z(t)}}}\ds
			\end{align*}
	so
	\begin{equation*}
		z_1(t)=nc_{t}\int_{0}^{t}z(t)\dt.
	\end{equation*}
	Similarly, we obtain
	\begin{equation*}
		z_2(t)\leq c\Delta\qquad z_3(t)\leq c\Delta.
	\end{equation*}
	So up to now we have
	\begin{equation*}
		z(t)\leq \tilde{c}_{t}\Delta+\tilde{c}_{t}\int_{0}^{T}z(t)\dt.
	\end{equation*}
	\begin{lemma}
		\emph{Grönwall's lemma}: if $u$ is a positive function such that
		\begin{equation*}
			u(t)\leq a+\int_{0}^{t}b(s)u(s)\ds
		\end{equation*}
		then
		\begin{equation*}
			u(t)\leq ae^{\int_{0}^{x}b(s)\ds}.
		\end{equation*}
	\end{lemma}
	We apply this and get
	\begin{equation*}
		z(t)\leq\tilde{c}_{t}\delta e^{\tilde{c}_{t}}
	\end{equation*}
	and therefore
	\begin{equation*}
		\sqrt{z(t)}\leq \tilde{c}_{t}\sqrt{\Delta}.
	\end{equation*}
\end{fancyproof}
We need to introduce the stochastic calculus in rder to obtain the Taylor expansion of $f(X_{t})$ at the second order. Take a \bwm{} $B$. I want to compare $(B_{t})^{2}$ (here $f(x)=x^{2}$). We know
\begin{equation*}
	(B_{t})^{2}=\lim_{N\to\infty}\left(\sum_{k=1}^{N}B_{t\frac{k}{N}}-B_{\frac{t(k-1)}{N}}\right)^{2}
\end{equation*}
so
\begin{align*}
	B_{t}&=B_{t}-B_{t\frac{N-1}{N}}+B_{t\frac{N-1}{N}}-\ldots+B_{0}\\
		&=lim_{N\to\infty}\sum_{k,j=1}^{N}\left(B_{t\frac{k}{N}}-B_{\frac{t(k-1)}{N}}\right)\left(B_{t\frac{j}{N}}-B_{\frac{t(j-1)}{N}}\right)\\
		&=lim_{N\to\infty}2\sum_{k\geq j}\ldots+\sum_{k=1}^{N}\left(B_{t\frac{k}{N}}-B_{\frac{t(k-1)}{N}}\right)^{2}\\
		&=\lim_{N\to\infty}2+\sum_{k=1}^{N}\left(B_{t\frac{k}{N}}-B_{\frac{t(k-1)}{N}}\right)B_{\frac{t(k-1)}{N}}+\frac{1}{N}\sum_{k=1}^{N}\left(B_{t\frac{k}{N}}-B_{\sqrt{N}\frac{t(k-1)}{N}}\right)^{2}.
\end{align*}
By the LLN this is equal to
\begin{equation*}
	2\int_{0}^{t}B_s\dif B_s+t.
\end{equation*}
The conclusion si that
\begin{equation*}
	B_{t}^{2}=2\int_{0}^{t}B_s\dif B_s+t.
\end{equation*}
If $t\to y(t)$ is a $\mathscr{C}^{2}$ continuous function with $y(0)=0$ then
\begin{equation*}
	y^{2}(t)=2\int_{0}^{t}y(s)y'(s)\ds.
\end{equation*}
\begin{theorem}
	Let $(X_{t})$ be the solution of the SDE
	\begin{equation*}
		\begin{cases}
			\dif X_t=b(t,X_t)\dt+\sigma(t,B_t)\dif B_t\\
			X_0=x_0
		\end{cases}
	\end{equation*}
	and let $f$ be a $\mathscr{C}^{2}$ continuous function. Then
	\begin{align*}
		f(X_{t})&=f(X_{0})+\int_{0}^{T}\left(f'(X_{s})b(X_{t})+\unmezz f''(X_{s})\sigma^{2}(X_{s})\right)\ds+\int_{0}^{t}f'(X_{s})\ubracketthin{\sigma(X_{s})\dif B_s}_{\dif X_{t}}.
	\end{align*}
\end{theorem}
An idea to improving the Euler scheme is assuming that $b$ and $\sigma$ don't depend on $t$.
\begin{equation*}
	\sigma(t,x)=\sigma(x)\qquad b(t,x)=b(x).
\end{equation*}
For the Euler-Maruyama method this means that
\begin{equation*}
	\int_{t}^{t+\Delta}b(X_{s})\ds\approx b(X_{t})\Delta
\end{equation*}
and
\begin{equation*}
	\int_{t}^{t+\Delta}\sigma(X_{s})\dif B_s\approx\sigma(X_{t})(B_{t+\Delta}-B_{t})\approx\sigma(X_{t})\sqrt{\Delta}G.
\end{equation*}
In order to improve the method we need to develop the stochastic integral
\begin{equation*}
	\sigma(X_{s})\approx\sigma(X_{t})+\int_{t}^{s}\sigma'(X_{n})\sigma(X_{n})\dif B_n.
\end{equation*}
We obtain the approximation scheme
\begin{equation*}
	\begin{cases}
		X^{\Delta}_{t_{n+1}}=X^{\Delta}_{t_{n}}+b(X^{\Delta}_{t_{n}})\Delta+\sigma\left(X^{\Delta}_{t_{n}}\right)\left(B_{t_{n+1}}-B_{t_{n}}\right)+\unmezz\sigma'\left(X^{\Delta}_{t_{n}}\right)\sigma\left(X_{t_{n}}\right)\left(\left(B_{t_{n+1}-B_{t_{n}}}\right)^{2}\Delta\right)\\
		X_{0}^{\Delta}=X_{0}.
	\end{cases}
\end{equation*}
This scheme is called the \emph{Milstein scheme}.
\begin{theorem}
	If $\sigma$ and $b$ are $\mathscr{C}^{2}$ continuous with bounded distances, then
	\begin{equation*}
		\ev{\sup_{t\leq T}\left|X^{\Delta}_{t}-X_{t}\right|^{2}}\leq c\Delta^{2}.
	\end{equation*}
\end{theorem}
This method is of order 1.
\begin{remark}
	If $\sigma=0$ this is simply the Euler method for ODEs.
\end{remark}
\subsection{Exact simulation method}
We consider 
\begin{equation*}
	\begin{cases}
		\dif X_t=b(t,X_t)\dt+\dif B_t\\
		X_0=x_0
	\end{cases}
\end{equation*}
Where $\sigma=1$. Remember that in rejection sampling, in order to generate $X$ with density $f$ we generate $Y$ with density $g$ and test
\begin{equation*}
	U\leq\frac{f(y)}{cg(y)}.
\end{equation*}
For any positive measure function $\psi$ we have
\begin{align*}
	\ev{\psi(x)}&=\int\psi(x)f(x)\dx\\
	&=c\ev{\psi(y)\frac{f(y)}{cg(y)}}\\
	&=c\ev{\psi(y)\indig{U\leq\frac{f(y)}{cg(y)}}}\\
	&=M:\ev{M}=1
\end{align*}
The Girsanov theorem tells us that for the solution $X_t$ of the SDE there exists a martingale $M_{t}$ such that
\begin{equation*}
	\ev{\psi(X_t)}=\ev{\psi(B_{t})M_{t}}
\end{equation*}
with $\ev{M_{t}}=1$. Using Ito's formula we can prove that
\begin{equation*}
	M_{t}=\expg{\beta(t,x)-\int_{0}^{t}\gamma(s,B_s)
		\dif B_s}
\end{equation*}
where $s(t,x)=\int_{x_0}^{x}b(t,y)\dy$ and $\gamma(t,x)=\frac{\partial \beta}{t}+\unmezz\left(\frac{\partial b}{\partial x}+b^{2}\right)$. We assume that
\begin{align*}
	&\beta\leq\beta_{\max}\to\text{constant}\\
	&0\geq\gamma\geq k\to\text{constant}.
\end{align*}
\begin{algobox}{Exact simulation}
	\begin{itemize}
		\item Step 1: generate $N\sim\mathsf{Pois}(kt)$ points uniformly on $(0,t)\times[0,k]$. Order the abscissa 
		\begin{equation*}
			(t_{1},y_{1}),(t_{2},y_{2}),\ldots,(t_{n},y_n)
		\end{equation*}
		with $t_1<t_2<\ldots<t_m$.
		 \item Step 2: generate a Gaussian vector that corresponds to
		\begin{equation*}
			\left(B_{t_{1}},B_{t_{2}},\ldots,B_{t_{n}},B_{T}\right).
		\end{equation*}
		\item Step 3: generaate $V\distunif{[0,1]}\independent N$.
		\item Step 4: test if
		\begin{align*}
			y_{i}&>\gamma(t_{i},B_{t_{1}})\every i\in{1,\ldots,N}\\
			Ve^{\beta_{\max}}&\leq e^{\beta(x,B_{t})}
		\end{align*}
		then set $z=B_{t}$, otherwise go to step 1.
	\end{itemize}
\end{algobox}
\appendix 
\chapter{Code}
This is the code shared by Professor Hermann. These are jupyter notebook that you can find on my \href{https://github.com/godblessourdeadkotatsu/lecture-notes-2024-25/tree/main/SP}{GitHub repo}. \faGithub
\includepdf[pages=1,pagecommand=\section{Simulation of \rv s},scale=0.9]{notebook/lesson1}
\includepdf[pages=2-4,scale=0.9]{notebook/lesson1}
\includepdf[pages=1,pagecommand=\section{Monte Carlo methods},scale=0.9]{notebook/lesson2}
\includepdf[pages=2,scale=0.9]{notebook/lesson2}
\includepdf[pages=1,pagecommand=\section{Simulation of \bwm},scale=0.9]{notebook/lesson3}
\includepdf[pages=2-7,scale=0.9]{notebook/lesson3}
\chapter{Homeworks}
These are the homeworks that we had to do this year. They are unreviewed, so take these at your own risk. For example, I am pretty sure the answer to the first question of the first homework is wrong.
\addcontentsline{toc}{section}{Homework 1}
\includepdf[pages=-,scale=0.9]{Homework 1/main}
\addcontentsline{toc}{section}{Homework 2}
\includepdf[pages=-,scale=0.9]{Homework 2/main}
\listoffigures  
\end{document} 
