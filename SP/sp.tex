...
\begin{equation*}
	B=(B^{1},B^{2},\ldots,B^{d})
\end{equation*}
where $B^{2},B_{2},\ldots$ are independent \bwm{} on $\R$ called \ul{coordinate processes}.
\begin{definition}
	...
	\begin{equation*}
		\ev_{s}f[W_{s+t}-W_{s}]=\int_{\R}\dx\left[\frac{e^{-\frac{x^{2}}{4t}}}{\sqrt{2\pi t}}f(x)\right]
	\end{equation*}
\end{definition}
\clearpage
\listoffigures  
