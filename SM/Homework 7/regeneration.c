...
#define LAMBDA_ARRIVAL 0.001  // Example lambda value for arrivals
#define LAMBDA_SHORT 0.005    // Example lambda value for short station
#define LAMBDA_LONG 0.008     // Example lambda value for long station
...
float lambda_arrival = LAMBDA_ARRIVAL;
float lambda_short = LAMBDA_SHORT;
float lambda_long = LAMBDA_LONG;
...
//MORE INITIALIZATION   
    ...
    //now the main loop. we need to iterate for each element of the event list, which will grow forever.
...
float exponential_random(float lambda)
{
    return lambda * -logf((float)rand() / RAND_MAX);
}
...
void add_event(Event new_event)
{
    ...
    if (event_count == event_capacity)
    {
        ...
        // using a temporary pointer allows us to use the list even if the realloc fails.
...
void process_event(Event current_event, int pos)
{
    ...
    if (RegPoint(current_event))
    {
        ...
        verbose ? printf("I found event of type %c, which is a regeneration point. I cleaned statistics and accumulated what I had to accumulate.\n\n",current_event.type) : 0;
    }

    // Process the event depending on its type
    verbose ? printf("Now processing: %c event, number %d in the event list, at time %f.\n",current_event.type,pos,current_event.timestamp) : 0;
    if (current_event.type == 'A')
    {
        // now we create and insert the next arrival
        Event next_arrival;
        next_arrival.timestamp =
            current_event.timestamp + exponential_random(lambda_arrival);
        next_arrival.type = 'A';
        add_event(next_arrival);
        verbose ? printf("Added the next arrival event %c, at time %f\n",next_arrival.type,next_arrival.timestamp) : 0;
        ...
        if (last_departure_timestamp == -1.0)
        {
            current_departure.timestamp =
                ...
                exponential_random(lambda_short); // empty queue
        ...
        else
        {
            current_departure.timestamp =
                ...
                exponential_random(lambda_short); // someone in the queue
        }
        // insert the fucking event
        add_event(current_departure);
        verbose ? printf("Added the departure event %c (corresponding to our current event %d) at time %f\n",current_departure.type,pos,current_departure.timestamp) : 0;
        ...
        if (coin_flip <= 0.2)
        {
            ...
            if (last_departure_timestamp == -1.0)
            {
                current_departure_long.timestamp =
                    current_event.timestamp +
                    exponential_random(lambda_long); // empty queue
                    verbose ? printf("But at least there was no one in queue for the long station") : 0;
            }
            else
            {
                current_departure_long.timestamp =
                    last_departure_timestamp +
                    exponential_random(lambda_long); // someone in the queue
            }
            // insert the event
            add_event(current_departure_long);
            verbose ? printf("\nAdded event %c at time %f\n",current_departure_long.type,current_departure_long.timestamp) : 0;
                ...
                */
            cycle_waiting_time_long_station += current_departure_long.timestamp - current_event.timestamp;
        }
    }
    ...
    verbose ? printf("Done processing event %c, number %d in the list, at time %f.\n",current_event.type,pos,current_event.timestamp) : 0;
    verbose ? printf("-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*\n\n") : 0;
}
...
void CollectRegStatistics(double *cycle_waiting_time_short_station,
                          ...
                          double *total_waiting_time_long, int *cycle_count)
{
    *total_waiting_time_short += *cycle_waiting_time_short_station; // Accumulate waiting time for short
                                                                   // station
    *total_waiting_time_long += *cycle_waiting_time_long_station;   // Accumulate waiting time for short station
