...
void process_event(Event* event) {
    ...
    switch (event->type) {
        case ARRIVAL: {
            ...
            machines_available--;
            double departure_time = event->timestamp + generate_exponential_random(LAMBDA_SHORT);
        ...
        case SHORT_REPAIR: {
            if (((double)rand() / RAND_MAX) <= PROBABILITY_LONG_REPAIR) {
                if (!long_repair_busy) {
                    ...
                    double long_repair_time = event->timestamp + generate_exponential_random(LAMBDA_LONG);
